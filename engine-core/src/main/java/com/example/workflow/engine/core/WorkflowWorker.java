...
public class WorkflowWorker {
    ...
    private void processActivity(ActivityExecution activity) {
        ...
        // 2. 컨텍스트 생성
        // TODO: ActivityExecution 데이터를 기반으로 실구현 WorkflowContext를 생성하는 ContextFactory 연동 필요
        ...
        try {
            ...
            // 3. 리플렉션을 통한 메서드 호출
            // 입력 파라미터가 있을 경우 JSON 역직렬화 필요 (현재는 문자열 그대로 전달 또는 생략 가정)
            Object[] args = (activity.inputData() != null) ? new Object[]{activity.inputData()} : new Object[0];
            // 파라미터 타입 매칭 로직은 향후 정교화 필요
            ...
            // 5. 실패 시 재시도 정책 적용
            // TODO: @Activity의 retryCount, backoffSeconds 설정을 읽어 지수 백오프(Exponential Backoff)를 적용한 재시도 스케줄링 필요
            taskExecutor.fail(context, cause, Duration.ofSeconds(metadata.annotation().backoffSeconds()));
        }
    }
