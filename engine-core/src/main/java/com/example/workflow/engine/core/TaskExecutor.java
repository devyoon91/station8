 ...
 * - 엔진 런타임은 본 인터페이스의 구현체를 통해 실제 실행/전이/체크포인트를 처리한다.
 */
public interface TaskExecutor {

    /**
     * 컨텍스트의 현재 액티비티를 실행한다. 구현체는 @Activity 메타데이터(재시도 등)를 해석할 수 있다.
     */
    void executeCurrent(WorkflowContext context);

    /**
     * 다음 액티비티를 스케줄링한다. 보통 현재 트랜잭션 커밋 이후 비동기로 실행되도록 큐잉한다.
     *
    ...
    /**
     * 현재 액티비티를 성공 처리하고 결과를 컨텍스트/스토리지에 반영한다.
     *
    ...
    /**
     * 현재 액티비티를 실패 처리하고, 재시도가 필요할 경우 backoff 지연을 반영한다.
     *
     * @param context 실행 컨텍스트
     * @param error 실패 원인
    ...
    /**
     * 체크포인트 스냅샷을 영속 저장한다. 구현체는 JSON 직렬화 등 구체 방식을 자유롭게 선택한다.
     */
