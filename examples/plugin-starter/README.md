...
#317 의 GitHub Actions가 main 머지마다 SNAPSHOT을 GitHub Packages로 publish하므로, 외부 개발자는 저장소 클론 없이도 SDK를 잡을 수 있다. PAT(권한: `read:packages`) 한 번만 발급해두면 된다.

`~/.gradle/gradle.properties`:

```properties
gpr.user=<본인 GitHub username>
gpr.token=<PAT (read:packages)>
```

starter `build.gradle`의 `repositories` 블록에 다음 추가:

```groovy
maven {
    name = 'GitHubPackages-Station8'
...
정식 `0.1.0` 릴리스 (#318) 후에는 Maven Central에서 anonymous read가 가능해져 위 PAT 설정도 불필요해진다.
