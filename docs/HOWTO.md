...
1. **명시 비밀번호** — 환경변수 또는 properties로 주입. `.env.example`을 복사해 시작:
   ...
   ```properties
   station8.initial-admin.username=admin
   station8.initial-admin.password=Hello!1234
   ```
   `docker compose`는 `docker/.env` 또는 root `.env`를 자동 로드 (`cp .env.example docker/.env`).

   > #232 결함 수정 이전(2026-05-14 이전)에 빌드된 이미지는 property 경로가 `station8.security.initial-admin.*`이라 `STATION8_INITIAL_ADMIN_*` env가 바인딩되지 않음 — 반드시 `up --build`로 최신 이미지를 빌드해야 함.
...
### 매 부팅마다 출력되는 InitialAdmin 상태 (#232 옵션 C)

부팅 시 INFO 한 줄로 admin 상태를 항상 출력 — 운영자가 비밀번호 출처/적용 여부를 즉시 파악 가능:

```
InitialAdmin: username=admin, status=seeded, source=configured (env STATION8_INITIAL_ADMIN_PASSWORD or properties)
InitialAdmin: username=admin, status=seeded, source=auto-generated
InitialAdmin: username=admin, status=pre-existing, source=ignored (admin already in DB)
InitialAdmin: username=admin, status=skip,  source=no password configured and other users exist
```

볼륨 영속(`mariadb_data`) 환경에서 두 번째 부팅부터는 항상 `pre-existing` 줄이 보임 — 비밀번호 분실 시 `down -v` + 재시드 필요.
...
| 권한 | 의미 |
|---|---|
| `READ` | 정의/인스턴스 조회 — 라인 정의 list / Dashboard / DLQ / Schedules 페이지에서 가시성 필터로 enforce (#159) |
...
**비범위 (별도 follow-up 이슈로)**: 그룹/팀 (#160) / 감사 로그 (#161) / LDAP 연동 (#162) / 태그 prefix 권한 (#170).

#### 5.0.1.1. READ enforcement — 가시성 필터 (#159)

라인 정의 list / Dashboard / DLQ / Schedules 페이지는 **현재 사용자가 볼 수 있는 정의만** 노출.

| 페이지 | 필터 |
|---|---|
| `/line/definitions` | 정의 행 + Tag cloud + Tag/Name 필터 모두 가시 정의 한정 |
| `/line/dashboard` | `WORKFLOW_NAME IN (...)` SQL 필터로 인스턴스 행 제한 |
| `/line/dlq` | 동일 패턴 — 가시 workflow의 DLQ 항목만 |
| `/line/schedules` | 가시 정의 ID에 묶인 스케줄만 |

**가시성 규칙** (`LineAclService.visibleDefinitionIds` / `visibleWorkflowNames`):
1. 글로벌 `ROLE_ADMIN` → 필터 미적용 (모든 행 노출)
2. 인증 안 된 요청 → 빈 set (아무것도 노출 안 함)
3. 인증된 USER → ACL entry 0건인 legacy 정의 ∪ 사용자 본인이 명시 grant 보유한 정의

**구현 노트**:
- ACL 데이터는 한 페이지당 2회 SQL(`findDefinitionIdsWithAcl` + `findDefinitionIdsForUser`) — 캐싱 없음
- Active 정의 ≤ 10000 가정 (#142와 동일 가정) — 더 큰 환경은 별도 마이그레이션
- Header stats(예: dashboard "RUNNING count")는 1차에서 글로벌 유지 — 가시 통계는 follow-up
...
`s3://` URI는 AWS S3 + MinIO / Ceph 같은 S3-호환 사내 객체저장소를 모두 cover. credential의 schema에 `endpoint`만 사이트 endpoint로 박으면 됨.

```json
{
  "uri": "s3://orders/inbox/2025-01-15.json",
  "format": "json",
  "credentialId": "minio-inbox"
}
```

vault credential 등록 (MinIO 예):

```
type:  s3_access_key
value: <SecretAccessKey>
schema: {"accessKeyId":"minioadmin","endpoint":"https://minio.internal:9000","region":"us-east-1","pathStyle":true}
```

AWS S3 직접 호출이면 `endpoint` 비우고 `pathStyle` 도 안 박으면 됨. MinIO / Ceph는 보통 `pathStyle: true` 필요 (virtual-host style 비호환).

큰 파일 (>100MB) streaming / 디렉토리 list 모드 / IAM instance profile 자동 인증은 별도 sub-issue로 진행. CSV format도 마찬가지.
...
# 외부 HTTP 호출 정책 (#289) — 자세한 가이드는 docs/HTTP_POLICY.md
station8.http.policy=                      # permissive 명시 시 모든 검증 off (운영 금지). 비우면 default
...
# S3 backend (#297) — AWS / MinIO / Ceph 등 S3-호환. endpoint/region/pathStyle은 credential schema에.
# 별도 properties 없음 — credential vault의 schema가 backend 라우팅까지 다 결정.
