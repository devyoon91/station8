...
| 테이블 | 역할 | 라이프사이클 |
|---|---|---|
| `U_LINE_DEFINITION` | DAG 메타데이터 + 버전 | 정의 등록 시 INSERT |
| `U_LINE_STATION` | 역 (activityNm, inputParams, 위치) | 정의 등록 시 INSERT |
| `U_LINE_TRACK` | 역 간 엣지 | 정의 등록 시 INSERT |
| `U_LINE_INSTANCE` | 라인 인스턴스 (status, input, output) | run-now 또는 스케줄 트리거 시 INSERT |
| `U_LINE_SCHEDULE` | Cron 스케줄 (next_run_dt, paused_fl) | 등록 시 INSERT, 폴러 틱마다 UPDATE |
| `H_LINE_ACTIVITY_EXECUTION` | 액티비티 실행 (status, retry, backoff, output) | 인스턴스 시작 시 역별 INSERT, 워커가 lock→run→complete/fail/retry로 UPDATE |
| `H_LINE_DLQ` | DLQ (NEW / REQUEUED / DISCARDED) | 최대 재시도 초과 시 INSERT, UI 액션으로 UPDATE |

상태 머신:

```
[H_LINE_ACTIVITY_EXECUTION.status]
...
PENDING ─── poll/lock ───▶ RUNNING ──┬─ success ───▶ COMPLETED
   ...
   └──────────────────────────────────┘
                                      │
                                      └─ fail (최대 재시도 초과) ─▶ FAILED + H_LINE_DLQ
```
...
LineDefinitionController.create(req)
  └─ LineDefinitionService.createDefinition(req)
       ...
       └─ JdbcLineDefinitionRepository
             ├─ INSERT U_LINE_DEFINITION
             ├─ INSERT U_LINE_STATION   (역 N개)
             └─ INSERT U_LINE_TRACK   (엣지 M개)
```
...
LineDefinitionController.run(id, body)
  └─ LineDefinitionService.runDefinition(id, inputData)
       ├─ INSERT U_LINE_INSTANCE  (status=RUNNING, start_dt=now)
       │
       └─ DagInterpreter.startInstance(definitionId, instanceId, inputData)
             ...
             └─ for each node:
                   ...
                   └─ ActivityRepository.createForNode(...)
                         └─ INSERT H_LINE_ACTIVITY_EXECUTION
```
  ...
  processActivity(activity)
    ...
    └─ method.invoke(bean, args)   ← 사용자 코드 실행
          ...
          └─ 실패:
               ...
               │    │            └─ INSERT H_LINE_DLQ + DlqNotifier.notify(...)
...
pollSchedules()
  ...
  └─ for each due schedule → triggerOne(schedule)
        ├─ INSERT U_LINE_INSTANCE  (input_data = schedule.inputData)
        ├─ DagInterpreter.startInstance(definitionId, instanceId, inputData)
...
LineWorker.moveToDlq()
  ├─ INSERT H_LINE_DLQ (status=NEW, error_message, stack_trace, ...)
...
엔진은 `H_LINE_ACTIVITY_EXECUTION`에 호출 결과를, Spring Batch는 `BATCH_JOB_EXECUTION`에 step/chunk 상세를 기록한다. `OUTPUT_DATA.jobExecutionId`로 연결.
...
| GET | `/line/dashboard` | `LineMonitoringController#dashboard` | `dashboard.mustache` | 인스턴스 목록 (`U_LINE_INSTANCE`) |
...
| GET | `/line/definitions` | `LineDefinitionPageController#list` | `definitions.mustache` | 활성 정의 목록 (`U_LINE_DEFINITION`) |
| GET | `/line/definitions/{id}` | `LineDefinitionPageController#preview` | `definition-preview.mustache` | 노선도(서브웨이 맵) SVG 미리보기 |
| GET | `/line/schedules` | `ScheduleController#list` | `schedules.mustache` | `U_LINE_SCHEDULE` |
| GET | `/line/dlq` | `LineMonitoringController#dlqList` | `dlq.mustache` | `H_LINE_DLQ` |
