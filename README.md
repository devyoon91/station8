...
- **간결한 SDK** — `@Workflow`, `@Activity` 어노테이션만으로 워크플로우 정의.
- **운영 UI** — Dashboard / Builder / Schedules / DLQ / Activity Catalog.
...
- `engine-core` — 엔진 코어. 어노테이션, DAG 인터프리터, 워커 폴링, DLQ.
- `service-app` — Spring Boot 애플리케이션. 컨트롤러, Mustache 뷰, 비즈니스 액티비티.
...
- [GitHub Issues](https://github.com/devyoon91/simple-workflow-engine/issues) · [Milestones](https://github.com/devyoon91/simple-workflow-engine/milestones)

## 코어 인터페이스

특정 DB 기술에 종속되지 않는 코어 계약.

- `@Workflow` — 워크플로우 클래스 식별 어노테이션 (선택적 이름).
- `@Activity` — 액티비티 메서드 식별 + 재시도 정책 (`retryCount`, `backoffSeconds`).
- `WorkflowExecutor` — 인스턴스 시작/재개 인터페이스.
- `WorkflowContext` — 실행 맥락. 입력/출력, 시도 횟수, 속성, 체크포인트.
- `TaskExecutor` — 현재 액티비티 실행, 다음 단계 스케줄, 완료/실패 처리.
- `WorkflowWorker` — DB 폴링(SKIP LOCKED) + 스레드 풀 기반 비동기 실행기.
