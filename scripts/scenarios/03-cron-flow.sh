...
step "POST /api/line/schedules/$SCH_ID/run-now (즉시 트리거)"
out=$(http POST "/api/line/schedules/$SCH_ID/run-now")
status=$(echo "$out" | head -1)
[[ "$status" != "201" ]] && { fail "run-now status $status"; exit 1; }
INST_ID=$(echo "$out" | tail -n +2 | jq -r '.instanceId')
pass "Triggered instanceId=$INST_ID"

step "Cleanup: DELETE /api/line/schedules/$SCH_ID"
http DELETE "/api/line/schedules/$SCH_ID" > /dev/null
